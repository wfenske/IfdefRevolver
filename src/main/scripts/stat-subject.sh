...
perc_abfuncs_perc_loac_loc=$(printf '%s\n' "$files_allfuncs_loc_loac_abfuncs"|csvsql -d ',' -q '"' --tables t --query "SELECT CAST(ROUND((100.0 * ab_functions) / total_functions, 0) AS INT) perc_abfuncs,CAST(ROUND((100.0 * total_loac) / total_loc, 0) AS INT) perc_loac, total_loc AS loc FROM t" /dev/stdin)

log_debug "$perc_abfuncs_perc_loac_loc"
