...
aggregate()
{
    csvsql -y 10000 --query \
	   "SELECT FUNCTION_SIGNATURE,FILE,FUNCTION_LOC,COMMIT_ID,COUNT(COMMIT_ID) as HUNKS,SUM(LINE_DELTA) as LINE_DELTA,SUM(LINES_DELETED) as LINES_DELETED,SUM(LINES_ADDED) as LINES_ADDED
       	   FROM stdin
       	   GROUP BY FUNCTION_SIGNATURE,FILE,FUNCTION_LOC,COMMIT_ID"
}
