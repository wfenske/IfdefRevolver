...
options <- list(
    make_option(c("-p", "--project")
              , help="Name of the project whose data to load.  We expect the input R data to reside in `results/<projec-name>/allData.rdata' below the current working directory. If `-p' is omitted, you have to specify the .rdata file as a positional argument."
              , default = NULL
                )
...
printHeader <- function() {
    dummy <- printf('System,I,D,SpearmanRho,p,Magnitude\n')
}
...
for (dep in c("COMMITS"
    ...
    for (indep in c("LOC"
        ...
        dummy <- printRhoRow(allData, indep, dep, systemName)
    }
}
