...
loc_percent_changes=$(csvsql -d ',' -q '"' --query 'select abs(old.function_loc-new.function_loc) * 100.0 / old.function_loc as perc_loc_diff from old join new on old.function_signature=new.function_signature and old.file=new.file where old.commits > 0' --tables old,new "$old_ch_file" "$new_ch_file") || exit $?
median_loc_percent_change=$(printf '%s\n' "$loc_percent_changes"|averages.R -c perc_loc_diff -H) || exit $?

if $PRINT_HEADER
then
    printf 'DATE,FUNC_BEFORE,FUNC_NOW,FUNC_IDENTICAL,FUNC_ADDED,FUNC_REMOVED,FUNC_PERC_ADD_REM,FUNC_MEDIAN_PERC_LOC_CHG\n'
fi

LC_NUMERIC=C; export LC_NUMERIC
printf '%s,%d,%d,%d,%d,%d,%d,%.2f\n' "${NEW:?}" ${oldc:?} ${newc:?} ${identicalc:?} ${addedc:?} ${removedc:?} ${percent_add_rem:?} "${median_loc_percent_change:?}"
